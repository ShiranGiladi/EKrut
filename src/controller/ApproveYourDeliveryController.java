...
/**
 * This class allows the user to approve an order that was delivered to them.
 ...
 *
 */
public class ApproveYourDeliveryController {
    @FXML
    ...
    /**
	* This method closes the current window ("Approve Your Delivery") and opens the window "Distant Client".
	* @param event
	* @throws IOException
	*/ 
    @FXML
	...
	/**
	 * This method is for a button that the client clicks on to approve that they have got their order.
	 * If the order already have been approved, an appropriate error message will appear on the screen.
	 * Otherwise, a pop up window will appear on the screen to let the client know that the update about the delivery being collected was successful.
	 * @param event
	 */
    @FXML
    void getApproveBtn(ActionEvent event) {
    	ClientUI.chat.accept("setConfirmationDateForDeliveryOrder " + txtOrderCode.getText());
    	switch(ChatClient.answerMsg) {
    	case "Error":
    		errorMsg.setText("Error, delivery already approved");
    		break;
    	case "Approve":
    		popUpWindow();
    		break;
    ...
    /**
	* This method opens a pop up window, that lets the client know that the update about the delivery being collected was successful.
	*/
    private void popUpWindow() {
		Scene popUp = new Scene(initPane(), 370, 155);
		Stage popUpStage = new Stage();
		popUpStage.setTitle("Delivery is collected");
		popUpStage.initStyle(StageStyle.UNDECORATED);
		popUpStage.setScene(popUp);
		popUpStage.show();
	}
    /**
	* This method sets the pop up window that lets the client know that the update about the delivery being collected was successful.
	...
	*/
    private Pane initPane() {
		...
		Label label = new Label("Delivery is collected");
    ...
    void getOkBtn(ActionEvent event) {
    	...
    	if(txtOrderCode.getText().trim().isEmpty())
    		...
    		ClientUI.chat.accept("getOrderDetailsForUser " + ChatClient.user.getId() + " delivery " + txtOrderCode.getText() + " " + ChatClient.user.getNote());
