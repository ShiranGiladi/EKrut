...
/**
 * This class allows the area manager to approve a client account for a client.
 * The client data is in the system from an external "user management" system, and the account is already created by the service representative.
 * Also from the window this class represents, the area manager can go back to their home page, or exit the system altogether.
 */
public class ClientsToApproveController {
	...
	@FXML
    private Button ApproveUserBtn;

    @FXML
    private Text errorMsg;
	
	@FXML
    private TableView<Clients> ClientsToApprove;
	  
    @FXML
    private TableColumn<Clients, String> Approve;
    ...
    public void initialize() {
		...
		if (!returnList.isEmpty()) {
			ClientsToApprove.setItems(returnList);
			ClientsToApprove.refresh();
		}    	
    }
    /**
    ...
    public ObservableList<Clients> getClients(){
		...
		ClientUI.chat.accept("getUsersAreNotApprove " + ChatClient.user.getNote());
		if (!ChatClient.answerMsg.equals("Empty")) {
			String msg = ChatClient.answerMsg.replace(" ", "");
    ...
    /**
     * This method approves an account for the client (the client's data is in the system from the external "user management" system,
     * 													and the account is already created by the service representative).
     * In case of any error, an appropriate error message will appear on the screen,
     * In case of success, a pop up window will appear on the screen to let the area manager know that the account was successfully approved.
     * @param event
     */
    @FXML
    void getApproveUserBtn(ActionEvent event) throws IOException {
    	Clients c = ClientsToApprove.getSelectionModel().getSelectedItem();
		if (c == null) {
			errorMsg.setText("Please choose an account.");
			return;
		}
		ClientUI.chat.accept("approveByManager " + c.getId());

		switch (ChatClient.answerMsg) {
		case "Error":
			errorMsg.setText("Failed approve by manager.");
			break;
		case "Update":
			ObservableList<Clients> returnList = getClients();
			if (returnList.isEmpty()) {
				ClientsToApprove.getItems().clear();
			} else {
				ClientsToApprove.setItems(returnList);
			}
			ClientsToApprove.refresh();
    ...
    /**
	* This method opens a pop up window, that lets the area manager know that the client's account was successfully approved.
	*/
	private void popUpWindow() {
		Scene popUp = new Scene(initPane(), 370, 155);
		Stage popUpStage = new Stage();
		popUpStage.setTitle("Account approved successfully");
		popUpStage.initStyle(StageStyle.UNDECORATED);
		popUpStage.setScene(popUp);
		popUpStage.show();
	}
	/**
	* This method sets the pop up window that lets the area manager know that the client's account was successfully approved.
	...
	*/	
    private Pane initPane() {
		...
		Label label = new Label("Account approved successfully.");
    ...
    /**
	* This method closes the current window ("Clients To Approve") and opens the window "Area Manager".
	* @param event
	* @throws IOException
	*/ 
    @FXML
